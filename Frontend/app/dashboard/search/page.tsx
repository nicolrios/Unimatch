...
export default function SearchPage() {
  ...
  const handleMatchRequest = async (targetId: string) => {
    ...
    } catch (error) { console.error(error) } finally { setSendingId(null) }
  }
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="relative group mb-20">
          ...
          <div className="relative flex items-center bg-[#0a0f1a]/80 backdrop-blur-2xl rounded-[2.5rem] p-3 border border-white/10 shadow-2xl">
            ...
            <input value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSearch()} placeholder="Escribe un tema (ej: Cálculo, SQL, Java)..." className="w-full bg-transparent p-6 outline-none text-sm font-bold placeholder:text-gray-700 tracking-tight" />
            <button onClick={handleSearch} className="bg-gradient-to-r from-blue-600 via-violet-600 to-pink-600 px-12 py-5 rounded-[2rem] font-black text-xs uppercase tracking-[0.2em] transition-all duration-500 hover:scale-[1.02] shadow-xl shadow-blue-900/30">{loading ? "Buscando..." : "Buscar"}</button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {loading ? (
                ...
                <div key={m.id} className="group relative bg-[#0a0f1a]/60 backdrop-blur-xl border border-white/5 rounded-[3rem] p-8 transition-all duration-500 hover:border-blue-500/30">
                  <div className="flex flex-col items-center text-center">
                    ...
                    {isSent ? (
                      ...
                      <button onClick={() => handleMatchRequest(m.id)} disabled={sendingId === m.id} className="w-full py-4 bg-white/5 hover:bg-gradient-to-r hover:from-pink-600 hover:to-blue-600 border border-white/10 rounded-2xl font-black text-[9px] tracking-[0.2em] uppercase transition-all duration-500">{sendingId === m.id ? 'Sincronizando...' : 'Hacer Match'}</button>
                    )}
                  </div>
                </div>
              );
            ...
            <div className="col-span-full py-32 border-2 border-dashed border-white/5 rounded-[4rem] flex items-center justify-center opacity-30"><p className="text-[10px] font-black tracking-[0.4em] uppercase">No se detectaron nodos con ese tema</p></div>
          )}
        </div>
      </div>
    </div>
