...
export default function SearchPage() {
  ...
  // Cargamos sugerencias iniciales para dar vida a la pantalla
  useEffect(() => {
    if (!user) return
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="relative group mb-20">
          ...
          <div className="relative flex items-center bg-[#0a0f1a]/80 backdrop-blur-2xl rounded-[2.5rem] p-3 border border-white/10 shadow-2xl">
            {/* Ícono de búsqueda estilizado (SVG) */}
        ...
        {/* SECCIÓN DE RESULTADOS / SUGERENCIAS */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {loading ? (
              ...
              suggestions.map((m: any) => (
                <div key={m.id} className="group relative bg-[#0a0f1a]/50 backdrop-blur-xl border border-white/5 rounded-[3rem] p-8 hover:border-pink-500/30 transition-all duration-500 hover:shadow-[0_0_40px_rgba(236,72,153,0.05)]">
              ...
              <div className="col-span-full py-32 border-2 border-dashed border-white/5 rounded-[4rem] flex flex-col items-center opacity-30">
                <p className="text-[10px] font-black tracking-[0.4em] uppercase">No se detectaron coincidencias en este sector de la red</p>
              </div>
            )}
          </div>
        </div>
